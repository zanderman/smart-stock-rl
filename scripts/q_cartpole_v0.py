...
def train(agent: ss.algorithms.qlearning.q.Q, env: gym.Env, goal: float, window: int, n_episodes: int = 1000) -> Tuple[List[float], bool]:
...
def main():
    ...
    policy = ss.algorithms.qlearning.policies.FourierPolicy(
        env.action_space, 
    ...
    agent = ss.algorithms.qlearning.q.Q(env, policy, gamma, alpha)
